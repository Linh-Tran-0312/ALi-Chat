import { Accordion, AccordionDetails, AccordionSummary, Avatar, Box, Button, Typography } from '@material-ui/core';
...
const ChatInfo = () => {
    ...
    const addMember = (member) => {
        if (!conversation.people.find(person => person === member._id)) {
            ...
            emitAddMember({ conversationId: conversation._id, people: conversation.people, userId: member._id })
        }
    };

    const removeMember = (memberId) => {
        const people = conversation.people.filter(member => member !== memberId)
       emitRemoveMember({ conversationId: conversation._id, people, userId: memberId, isRemoved: true })
    }

    const leaveGroup = () => {
        setIsRemoving(true);
        const people = conversation.people.filter(member => member !== userId)
        emitRemoveMember({ conversationId: conversation._id, people, userId, isRemoved: false })
    }
                                    ...
                                    <div style={{ width: '100%', textAlign: 'center' }}>
                                        {
                                            ...
                                            (<Box width="100%" textAlign="left" mb={2}>
                                                <MemberSearch list={memberResults} clear={handleClear} handleSearchMem={handleSearchMem} selectMember={addMember} searchWidth="100%" resultWidth="100%" />
                                            </Box>)
                                        }
                                        {
                                        ...
                                        }
                                        {
                                            ...
                                            (<Box my={2}>
                                                {
                                                    isRemoving ? <CircularProgress color="secondary" size={20}/> : <Button variant="contained" size="small" color="default" onClick={leaveGroup} endIcon={<ExitToAppIcon />}>Leave Group</Button>
                                                }
                                            </Box>) : null
                                        }
                                    </div>
